...
interface IndividualWatchlistRowProps {
...
export function IndividualWatchlistRow({
  company,
  onQuickAddTag,
  onQuickAddNote,
  onRemoveFromWatchlist,
}: IndividualWatchlistRowProps) {
  ...
  return (
    <tr className="hover:bg-secondary/50 transition-colors border-b">
      ...
      <td className="p-4 text-right">
        <span className="font-mono text-sm">{formatMarketCap(company.market_cap)}</span>
      </td>

      {/* Last Note Date */}
      <td className="p-4">
        <span className="text-xs text-muted-foreground">{formatLastNote(company.last_note_date)}</span>
      </td>
      ...
      <td className="p-4">
        <div className="flex gap-2">
          <Button
            ...
            onClick={(e) => {
              e.stopPropagation();
              if (onQuickAddTag) onQuickAddTag();
            }}
            className="gap-1"
          >
          ...
          <Button
            ...
            onClick={(e) => {
              e.stopPropagation();
              if (onQuickAddNote) onQuickAddNote();
            }}
            className="gap-1"
          >
