...
|----------|-------------|
| `POST /chunk_search` | Standard chunk search (returns all results at once) |
| `POST /chunk_search_streaming/stream` | Streaming chunk search with real-time pipeline status updates |
...
## Streaming Chunk Search API

The streaming endpoint (`POST /chunk_search_streaming/stream`) provides real-time updates as each pipeline component completes. This is useful for showing progress indicators in the UI.

### How Streaming Works

The streaming endpoint uses Server-Sent Events (SSE) to broadcast updates. Each event is a JSON object with a `type` field indicating the event type.

### Event Types

| Type | Description | When Emitted |
|------|-------------|--------------|
| `component_status` | Pipeline component completed | After each component finishes |
| `queries` | Extracted/expanded queries | After SearchQueryExpander completes |
| `complete` | Final results | When pipeline is fully complete |
| `error` | Error occurred | On any pipeline error |
...
interface ComponentStatusEvent {
  ...
  data: {
    component: string;      // Component name (e.g., "SearchQueryExpander", "BM25Retriever")
...
interface CompleteEvent {
  type: "complete";
  data: ChunkSearchResponse;  // Same as non-streaming response
}
```
...
The streaming endpoint broadcasts status for each component:

| Order | Component | Description | Typical Time |
|-------|-----------|-------------|--------------|
| 1 | SearchQueryExpander | Extracts and expands user query | 200-500ms |
| 2 | EmbeddingComponent | Generates query embeddings | 100-200ms |
| 3 | BM25Retriever | Full-text search retrieval | 100-300ms |
| 4 | SemanticRetriever | Vector similarity search | 100-300ms |
| 5 | EnhancedResultMerger | Merges and reranks results | 500-2000ms |
| 6 | ChunkSearchGrouper | Groups results by company/document | 10-50ms |

### Frontend Integration for Streaming

**JavaScript Example:**
```javascript
const eventSource = new EventSource('/chunk_search_streaming/stream', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({ text: "revenue growth guidance" })
});

// Note: EventSource doesn't support POST directly.
// Use fetch with ReadableStream instead:

async function streamingChunkSearch(query) {
  const response = await fetch('/chunk_search_streaming/stream', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    ...
    for (const line of lines) {
      ...
      switch (event.type) {
        case 'component_status':
          console.log(`${event.data.component} completed in ${event.data.execution_time_ms}ms`);
          // Update UI progress indicator
          updateProgressBar(event.data.component, event.data.execution_time_ms);
          break;

        case 'queries':
          console.log('Queries expanded:', event.data);
          // Show expanded queries early if desired
          break;
        ...
        case 'error':
          console.error('Search failed:', event.data);
...
function useStreamingChunkSearch() {
  const [steps, setSteps] = useState<PipelineStep[]>([]);
  const [results, setResults] = useState<ChunkSearchResponse | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const search = useCallback(async (query: string, options?: Partial<ChunkSearchRequest>) => {
    setIsLoading(true);
    setSteps([]);
    setResults(null);
    setError(null);
  ...
  return { search, steps, results, isLoading, error };
}
```

### UI Progress Display Example

```tsx
function SearchProgress({ steps }: { steps: PipelineStep[] }) {
  const allSteps = [
    { name: 'SearchQueryExpander', label: 'Expanding Query' },
    { name: 'EmbeddingComponent', label: 'Generating Embeddings' },
    { name: 'BM25Retriever', label: 'Text Search' },
    { name: 'SemanticRetriever', label: 'Semantic Search' },
    { name: 'EnhancedResultMerger', label: 'Merging & Reranking' },
    { name: 'ChunkSearchGrouper', label: 'Grouping Results' },
  ];

  const completedSteps = new Set(steps.map(s => s.component));
  const stepTimes = Object.fromEntries(steps.map(s => [s.component, s.execution_time_ms]));

  return (
    <div className="pipeline-progress">
      {allSteps.map((step, i) => {
        const isCompleted = completedSteps.has(step.name);
        const time = stepTimes[step.name];

        return (
          <div key={step.name} className={`step ${isCompleted ? 'completed' : 'pending'}`}>
            <span className="step-icon">
              {isCompleted ? '✓' : '○'}
            </span>
            <span className="step-label">{step.label}</span>
            {isCompleted && (
              <span className="step-time">{time.toFixed(0)}ms</span>
            )}
          </div>
        );
      })}
...
|----------|---------------------|
| Quick searches with loading spinner | `/chunk_search` (non-streaming) |
| Long searches needing progress feedback | `/chunk_search_streaming/stream` |
| Mobile apps with poor connectivity | `/chunk_search_streaming/stream` |
| Background/automated searches | `/chunk_search` (non-streaming) |
| Interactive search with step visualization | `/chunk_search_streaming/stream` |
